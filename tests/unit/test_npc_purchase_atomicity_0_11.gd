...
func run() -> bool:
	...
	var unknown := commerce.buy(&"shop:stillpoint_blacksmith_equipment", &"missing", 1, inventory, wallet)
	...
	var insufficient := commerce.buy(&"shop:stillpoint_blacksmith_equipment", &"improved_forge_hammer", 1, inventory, wallet)
	...
	wallet.restore_balance(100)
	var full_result := commerce.buy(&"shop:stillpoint_blacksmith_equipment", &"improved_forge_hammer", 1, full, wallet)
