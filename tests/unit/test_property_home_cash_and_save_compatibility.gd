...
func run() -> bool:
	...
	var saved_v2 := service.capture_save_data(10_000)
	ok = ok and int(saved_v2.get("section_version", 0)) == 2
	ok = ok and int(saved_v2.get("home_cash_balance", -1)) == 200
	service.free()

	var restored := PropertyBankService.new()
	ok = restored.restore_save_data(saved_v2, 10_000) and ok
	...
	var legacy_source := saved_v2.duplicate(true)
	restored.free()
