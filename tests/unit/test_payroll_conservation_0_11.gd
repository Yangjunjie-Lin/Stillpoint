...
func run() -> bool:
	...
	var state := EconomicTestHelper.worksite_state(100)
	...
	ok = ok and state.payroll_balance == 80 and actor.wallet.get_balance() == 30
	ok = ok and state.payroll_balance + actor.wallet.get_balance() == 110
