...
class MainMenu:
    ...
    def __init__(self):
        """Initialize some mainmenu parameters"""
        self.root = tk.Tk()
...
class ExplorationandShottingGame:
    """Main game class handling gameplay mechanics."""

    def __init__(self, menu=None, root=None):
        """Define some game page parameters"""
        ...
        # Load player image
        try:
            up_image = Image.open("player_up.png")
            up_image = up_image.resize((self.player_size, self.player_size))
            self.player_images['up'] = ImageTk.PhotoImage(up_image)
            ...
            for direction in directions:
                try:
                    image_path = f"player_{direction}.png"
                    image = Image.open(image_path)
                    image = image.resize((self.player_size, self.player_size))
                    self.player_images[direction] = ImageTk.PhotoImage(image)
                except Exception:
                    print("Warning: Could not load "
                          f"{direction} image, using up image instead")
                    self.player_images[direction] = self.player_images['up']

        except Exception:
            print("Error: Could not load player images")
        ...
        try:
            chase_image = Image.open("obstacle_chase.png")
            chase_image = chase_image.resize((
                self.obstacle_size,
                self.obstacle_size
            ))
            self.obstacle_images['chase'] = ImageTk.PhotoImage(chase_image)

            avoid_image = Image.open("obstacle_avoid.png")
            avoid_image = avoid_image.resize((
                self.obstacle_size,
                self.obstacle_size
            ))
            self.obstacle_images['avoid'] = ImageTk.PhotoImage(avoid_image)

            circle_image = Image.open("obstacle_circle.png")
            circle_image = circle_image.resize((
                self.obstacle_size,
                self.obstacle_size
            ))
            self.obstacle_images['circle'] = ImageTk.PhotoImage(circle_image)

        except Exception:
            print("Warning: Could not load obstacle images")
        ...
        # Create a empty list to store cheat inputs
        self.cheat_input = ""
        self.cheat_codes = {
            "godmode": "immortal",
            "rapidfire": "flash",
    ...
    def check_cheat_code(self):
        """Check whether the cheat code are used correctly"""
        # Check the code if in the list
        ...
        # Avoid the input too long
            if len(self.cheat_input) > 20:
                self.cheat_input = ""

    def activate_cheat(self, code):
        """Define some states when using cheat codes"""
        current_time = time.time()

        # Define the godmode state
        if code == "godmode":
    ...
    def show_cheat_message(self, message):
        """Show whether we are in cheating states"""
        ...
        cheat_message.geometry(
            f"{cheat_message_width}x{cheat_message_height}+{x}+{y}")
    ...
    def auto_save(self):
        """Auto save game state to file"""
        save_data = str({
            'player_name': self.player_name,
            'player_world_x': self.player_world_x,
            'player_world_y': self.player_world_y,
            'score': self.score,
            ...
            'shield_end_time': self.shield_end_time,
            'speed_boost_end_time': self.speed_boost_end_time,
            'score_multiplier': self.score_multiplier,
            'score_multiplier_end_time': self.score_multiplier_end_time,
            'weapon_type': self.weapon_type,
            'weapon_end_time': self.weapon_end_time,
            'difficulty_scale': self.difficulty_scale,
            'bullets': self.bullets,
            'timestamp': time.time(),
            'is_game_over': False  # Add game state flag
        })
    ...
    def auto_load(self):
        """Auto load game state from file"""
        try:
            with open('autosave.txt', 'r', encoding='utf-8') as f:
                save_data = eval(f.read())

            if time.time() - save_data.get('timestamp', 0) > 86400:
                return

            if save_data.get('is_game_over', False):
                return

            self.player_name = save_data['player_name']
            self.player_world_x = save_data['player_world_x']
            self.player_world_y = save_data['player_world_y']
            self.score = save_data['score']
            self.survival_score = save_data['survival_score']
            self.start_time = save_data['start_time']
            self.obstacles = save_data['obstacles']
            self.items = save_data['items']
            self.shield_active = save_data['shield_active']
            self.shield_end_time = save_data['shield_end_time']
            self.speed_boost_end_time = save_data['speed_boost_end_time']
            self.score_multiplier = save_data['score_multiplier']
            self.score_multiplier_end_time = (
                save_data['score_multiplier_end_time'])
            self.weapon_type = save_data['weapon_type']
            self.weapon_end_time = save_data['weapon_end_time']
            self.difficulty_scale = save_data['difficulty_scale']
            self.bullets = save_data['bullets']

            self.player_x = self.player_world_x - self.map_offset_x
            self.player_y = self.player_world_y - self.map_offset_y

        except Exception:
            print("No autosave found or failed to load")

    def update_movement(self):
        """Update player movement based on pressed keys and
        ...
        # Update player direction if there is movement
        ...
        # Keep last direction when not moving
        else:
            self.current_direction = self.last_direction

        # Apply movement if velocity exceeds threshold
        if abs(self.velocity_x) > 0.01 or abs(self.velocity_y) > 0.01:
            self.move_player(
    ...
    def toggle_boss_mode(self, event=None):
        ...
        if not self.is_boss_mode:
            ...
            label = tk.Label(spreadsheet_frame,
    ...
    def draw_parameters(self):
        ...
        self.canvas.create_text(
            20, 60, anchor='nw',
            text=f'Position: ({int(self.player_world_x)}, '
                 f'{int(self.player_world_y)}) ',
            fill='white',
            font=('Arial', 14)
        )
        ...
        self.canvas.create_text(
            20, 90, anchor='nw',
            text='Chaser: Chase   Runner: Avoid   Ambusher: Circle',
            fill='white',
            font=('Arial', 14)
        )
        ...
        self.canvas.create_text(
            20, 120, anchor='nw',
            text='Green Item: Shield(10s)   '
                 'Cyan Item: Speed(5s)   '
                 'Pink Item: Double Score(8s)',
            fill='white',
            font=('Arial', 14)
        )
        ...
        self.canvas.create_text(
            20, 150, anchor='nw',
            text=(
                'Left Click: Shoot (Cooldown: 0.5s)   '
                'ESC: Pause Game   B: Boss Key   '
                'F10: Show Parameters   F11: Fullscreen'),
            fill='white',
            font=('Arial', 14)
        )

    def show_pause_menu(self):
        """Show pause menu window with white background"""
        ...
        self.pause_menu.configure(bg='white')
        ...
        for button in (resume_button, restart_button, quit_button):
            button.bind('<Enter>',
                        lambda e, b=button: b.configure(bg='#f0f0f0'))
            button.bind('<Leave>',
                        lambda e, b=button: b.configure(bg='white'))
    ...
    def get_player_name(self):
        """Get player name input window with white background"""
    ...
    def update_difficulty(self):
        ...
        self.difficulty_scale = 1.0 + (
            (total_score // self.score_threshold) * 0.1
        )
        ...
        for obstacle in self.obstacles:
            ...
            obstacle['speed'] = obstacle['base_speed'] * min(
                2.0,
    ...
    def shoot(self, event):
        ...
        perpendicular_dx = -dy
        perpendicular_dy = dx  # Fixed the perpendicular calculation

        # Create base bullet properties
        bullet_props = {
        ...
        if self.weapon_type == "double":
            ...
            bullet1 = {
                'x': (self.player_world_x
                      + perpendicular_dx * self.double_shot_offset),
                'y': (self.player_world_y
                      + perpendicular_dy * self.double_shot_offset),
        ...
        # Remove the problematic double shot delay code
        if hasattr(self, 'last_double_shot'):
            delattr(self, 'last_double_shot')
    ...
    def update_bullets(self):
        ...
        for bullet in self.bullets:
            ...
            for obstacle in self.obstacles[:]:
                ...
                dx = bullet_screen_x - screen_ox
                dy = bullet_screen_y - screen_oy
            ...
            hit_condition = not hit or bullet['piercing']
            time_condition = (current_time -
                              bullet['created_time']) < bullet['lifetime']

            if hit_condition and time_condition:
    ...
    def draw_bullets(self):
        ...
        for bullet in self.bullets:
            ...
            distance = math.sqrt(
                (screen_x - self.player_x)**2 + (screen_y - self.player_y)**2)
    ...
    def check_item_collection(self):
        ...
        for item in self.items:
            ...
            distance = math.sqrt(
                (screen_ix - self.player_x)**2 +
                (screen_iy - self.player_y)**2)
    ...
    def move_player(self, dx, dy):
        ...
        if new_x < 100:  # Left edge
            ...
            self.map_offset_x = min(
                self.map_size - self.canvas_width,
                self.player_world_x - self.canvas_width + 100)

        if new_y < 100:  # Top edge
            self.map_offset_y = max(0, self.player_world_y - 100)
        elif new_y > self.canvas_height - 100:  # Bottom edge
            self.map_offset_y = min(
                self.map_size - self.canvas_height,
                self.player_world_y - self.canvas_height + 100)
    ...
    def update_obstacles(self):
        ...
        for obstacle in self.obstacles:
            ...
            if obstacle['behavior'] == 'chase':
                ...
                center_x = (
                    self.player_world_x +
                    math.cos(obstacle['angle']) * 100
                )
                center_y = (
                    self.player_world_y +
                    math.sin(obstacle['angle']) * 100
                )
    ...
    def is_collision(self, x, y):
        ...
        obstacle_radius = self.obstacle_size/2

        for obstacle in self.obstacles:
            screen_ox = obstacle['x'] - self.map_offset_x
            screen_oy = obstacle['y'] - self.map_offset_y

            distance = math.sqrt((x - screen_ox)**2 + (y - screen_oy)**2)
    ...
    def draw_items(self):
        ...
        for item in self.items:
            ...
            distance = ((screen_ix - self.player_x) ** 2 +
                        (screen_iy - self.player_y) ** 2) ** 0.5
            if distance <= self.visible_radius:
                color = {
                    'shield': '#00FF00',
                    'speed': '#00FFFF',
    ...
    def draw_status_effects(self):
        """Add the status in the status box"""
        margin = 10
        padding = 10
        y_start = 90
        x_start = margin
    ...
    def update_game(self):
        ...
        if not self.is_paused:
            ...
            current_time = time.time()
            if not hasattr(self, 'last_autosave'):
            ...
            for obstacle in self.obstacles:
                ...
                distance = ((screen_ox - self.player_x) ** 2 +
                            (screen_oy - self.player_y) ** 2) ** 0.5
                if distance <= self.visible_radius:
                    if self.obstacle_images and \
                       self.obstacle_images[obstacle['behavior']]:
                        self.canvas.create_image(
                            screen_ox,
                            screen_oy,
                            image=self.obstacle_images[obstacle['behavior']],
                            anchor='center'
                        )
                    else:
                        color = {
                        ...
                        self.canvas.create_rectangle(
                            screen_ox - self.obstacle_size/2,
                            screen_oy - self.obstacle_size/2,
                            screen_ox + self.obstacle_size/2,
                            screen_oy + self.obstacle_size/2,
                            fill=color
                        )
            current_image = self.player_images[self.current_direction]
            self.canvas.create_image(
                self.player_x,
                self.player_y,
                image=current_image,
            ...
            self.canvas.create_text(
                ...
                text=(
                    ...
                    f'(Game: {self.score} + '
                    f'Survival: {self.survival_score})'
                ),
                fill='white',
                font=('Arial', 20)
            )
            ...
            if self.is_collision(self.player_x, self.player_y):
                ...
                self.root.after(8, self.update_game)
        else:
    ...
    def game_over(self):
        ...
        # Save final state with game over flag
        save_data = str({
            'is_game_over': True,
            'timestamp': time.time()
        })
        try:
            ...
            print(f"Failed to save game over state: {str(e)}")
