...
func memory_candidate(npc_persistent_id: String, event: GameplayEvent) -> Dictionary:
	var high_salience_types := [&"npc_attacked", &"promise_made", &"quest_completed", &"entity_destroyed"]
	return {"owner_npc_persistent_id": npc_persistent_id, "source_type": "gameplay_event",
		...
		"subject_node_ids": [String(event.source_entity_id), String(event.target_entity_id)],
